...
export const useSupabase = () => {
  ...
  const loadUserProfile = async (userId) => {
    ...
    try {
      ...
      if (!data || error?.code === "PGRST116") {
        console.log(
          "User profile not found, attempting to create new profile...",
        );
        const {
          data: { user: authUser },
        ...
        if (authUser) {
          ...
          if (createError) {
            ...
            console.warn(
              "Could not create user profile automatically. Please run the SQL script to create it manually.",
            );
            return null;
          }
  ...
  const syncAllData = async () => {
    ...
    try {
      ...
      let expenses, prompts;
      try {
        expenses = await getExpenses();
      } catch (err) {
        console.warn("Error syncing expenses:", err.message);
      }
      try {
        prompts = await getAIPrompts();
      } catch (err) {
        console.warn("Error syncing AI prompts:", err.message);
      }
