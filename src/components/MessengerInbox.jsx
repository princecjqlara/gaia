...
import {
  ...
  generateNotes,
} from "../services/aiConversationAnalyzer";
...
const MessengerInbox = ({ clients = [], users = [], currentUserId }) => {
  ...
  const handleCreateSavedReply = async () => {
    ...
    try {
      const pageId = conversations[0]?.page_id;
      await facebookService.createSavedReply(
        pageId,
        newReplyTitle,
        newReplyContent,
        newReplyShortcut || null,
        "general",
        safeCurrentUserId,
      );
      setNewReplyTitle("");
  ...
  const handleAddTagToConversation = async (tagId) => {
    if (!selectedConversation) return;
    try {
      await facebookService.assignTag(
        selectedConversation.conversation_id,
        tagId,
        safeCurrentUserId,
      );
  ...
  const handleRemoveTagFromConversation = async (tagId) => {
    if (!selectedConversation) return;
    try {
      await facebookService.removeTag(
        selectedConversation.conversation_id,
        tagId,
      );
      loadConversationTags(selectedConversation.conversation_id);
  ...
  const handleArchiveConversation = async (conversationId) => {
    if (!confirm("Archive this conversation? It can be restored later."))
      return;
    try {
  ...
  const handleCreateTag = async () => {
    ...
    try {
      const pageId = conversations[0]?.page_id;
      await facebookService.createTag(
        pageId,
        newTagName,
        newTagColor,
        safeCurrentUserId,
  ...
  const handleAssignTag = async (conversationId, tagId) => {
    try {
      await facebookService.assignTag(conversationId, tagId, safeCurrentUserId);
      loadConversations?.(null, true);
    } catch (err) {
  ...
  const handleSendBulkMessage = async () => {
    ...
    try {
      ...
      alert(
        `Bulk message sent! ✅ ${result.sent} sent, ❌ ${result.failed} failed`,
      );
                      ...
                      onClick={(e) => {
                        ...
                        if (btn.type === "web_url") {
                          ...
                          if (btn.payload.startsWith("INQUIRY_PROPERTY_")) {
                            const propId = btn.payload.replace(
                              "INQUIRY_PROPERTY_",
                              "",
                            );
                            setMessageText(
                              `Hi, I'm interested in property ID: ${propId}. Can you provide more details?`,
                            );
                          }
                        }
                      }}
              ...
              <div style={{ padding: "0.75rem" }}>
                {el.buttons?.map((btn, bi) => (
                  <button
                    ...
                    onClick={() =>
                      btn.type === "web_url" && window.open(btn.url, "_blank")
                    }
                  >
                    {btn.title}
                  </button>
  ...
  return (
    <>
            ...
            >
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (!result) {
                    alert(
                      "Sync failed. Make sure you have connected a Facebook Page in Admin Settings → Facebook Integration.",
                    );
                  }
                }}
                disabled={syncing}
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    const result =
                      await facebookService.updateUnknownContactNames();
                    if (result.updated > 0) {
                      alert(
                        `✅ Fixed ${result.updated} contact names!\n\nNames extracted from message content.`,
                      );
                      await loadConversations();
                    } else {
                      alert(
                        'No names could be extracted from messages.\n\nTip: Click "Set Name" on individual contacts to manually enter names.',
                      );
                    }
                  } catch (err) {
          ...
          <div style={{ padding: "0.5rem 0.75rem" }}>
            ...
            {searchTerm.length >= 2 &&
              conversationSearchResults?.length > 0 && (
                ...
                >
                  🔍 Found {conversationSearchResults.length} contacts across
                  all pages
                </div>
              )}
                ...
                <span>
                  ...
                  {activeFilter === "all"
                    ...
                    : activeFilter === "no_reply"
                      ? "Awaiting Reply"
                      : activeFilter === "not_booked"
                        ? "Not Booked"
                        : activeFilter === "not_in_pipeline"
                          ? "Not in Pipeline"
                          : activeFilter === "in_pipeline"
                            ? "In Pipeline"
                            : activeFilter === "unread"
                              ? "Unread"
                              : activeFilter}
              ...
              {showFilterDropdown && (
                ...
                >
                  {[
                    ...
                    {
                      value: "status_appointment_booked",
                      label: "📅 Appointment Booked",
                    },
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {conv.last_message_from_page ? "You: " : ""}
                  ...
                  >
                    ...
                    >
                      ...
                      {conv.active_goal_id && (
                        <div
                          style={{
                            background: conv.goal_completed
                              ? "var(--success)"
                              : "var(--bg-tertiary)",
                            color: conv.goal_completed
                              ? "white"
                              : "var(--text-muted)",
                            borderRadius: "999px",
                            padding: "0.125rem 0.5rem",
              ...
              >
                ...
                <button
                  ...
                  style={{
                    marginLeft: "auto",
                    background:
                      selectedConversation.ai_enabled !== false
                        ? "linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)"
                        : "var(--bg-secondary)",
                    border:
                      selectedConversation.ai_enabled !== false
              ...
              >
                ...
                {messages.map((msg) => (
                  <div
                    ...
                    style={{
                      display: "flex",
                      justifyContent: msg.is_from_page
                        ? "flex-end"
                        : "flex-start",
                    }}
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          ...
                          disabled={
                            !newReplyTitle.trim() || !newReplyContent.trim()
                          }
                        >
                          Save
                        </button>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (value.startsWith("/") && value.length > 1) {
                        ...
                        const matchingReply = savedReplies.find(
                          (r) => r.shortcut?.toLowerCase() === shortcut,
                        );
                        if (matchingReply) {
            ...
            <>
              ...
              <div style={{ marginBottom: "1rem" }}>
                  ...
                  {loadingTags ? (
                    ...
                    conversationTags.map((tag) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            handleRemoveTagFromConversation(tag.id)
                          }
                          style={{
                            background: "none",
                            border: "none",
                ...
                >
                  ...
                  {tags
                    .filter(
                      (t) => !conversationTags.find((ct) => ct.id === t.id),
                    )
                    .map((tag) => (
              ...
              <div style={{ marginTop: "0.5rem" }}>
                <button
                  ...
                  onClick={() => {
                    if (
                      window.confirm(
                        `Are you sure you want to delete the conversation with ${selectedConversation.participant_name}? This action cannot be undone.`,
                      )
                    ) {
                      ...
                      if (messages && messages.length > 0) {
                        ...
                        try {
                          ...
                          setTransferForm((prev) => ({
                            ...prev,
                            businessName:
                              details?.businessName || prev.businessName,
                            contactDetails:
                              details?.phone ||
                              details?.email ||
                              prev.contactDetails,
                  ...
                  >
                    ...
                    <button
                      onClick={() => {
                        if (
                          window.confirm(
                            "Unlink this conversation from the client?",
                          )
                        ) {
                          linkToClient(
                            selectedConversation.conversation_id,
                ...
                >
                  <optgroup label="📋 Contact Status">
                    ...
                    <option value="selected">
                      Selected Contacts ({selectedConversations.size})
                    </option>
                  </optgroup>
                  ...
                  >
                    {[
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setBulkMessage((prev) => prev + item.var)
                        }
                        style={{
                          padding: "0.25rem 0.5rem",
                          background: "var(--primary-alpha)",
      ...
      {showAIChatbotSettings && (
        <AIChatbotSettings onClose={() => setShowAIChatbotSettings(false)} />
      )}

      {/* Best Times Overview Modal */}
      {showBestTimes && (
        <BestTimesOverview onClose={() => setShowBestTimes(false)} />
      )}
                  ...
                  {properties.map((p) => (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {p.location || p.address || "No location"}
                        </div>
                        <div
                          style={{
                            marginTop: "auto",
